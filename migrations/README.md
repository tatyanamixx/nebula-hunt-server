...
Миграции разделены на 5 основных файлов, каждый из которых отвечает за определенную группу таблиц:

1. **20250701000000-create-tables.js** - Основные таблицы (пользователи, состояния, токены)
2. **20250701000001-create-game-tables.js** - Игровые таблицы (галактики, артефакты)
3. **20250701000002-create-template-tables.js** - Таблицы шаблонов (апгрейды, задания, события)
4. **20250701000003-create-market-tables.js** - Рыночные таблицы (предложения, транзакции, комиссии)
5. **20250701000004-create-package-tables.js** - Таблицы пакетов (шаблоны пакетов, хранилище пакетов)
...
2. **Шаблоны** - TaskTemplate, EventTemplate, UpgradeNode и PackageTemplate являются глобальными шаблонами и не имеют прямой связи с конкретным пользователем. Они используются как справочники для создания экземпляров в UserState.
